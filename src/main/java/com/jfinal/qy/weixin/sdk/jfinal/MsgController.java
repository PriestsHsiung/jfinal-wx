...
import com.jfinal.qy.weixin.sdk.msg.kf.KfInMsgParser;
...
public abstract class MsgController extends Controller {
	...
	public void index() {
		...
		if (isKf()) {
		try {
			String s = getInMsgXml();
//			s = "<xml>" +
//					"<AgentType><![CDATA[kf_internal]]></AgentType>" +
//					"<ToUserName><![CDATA[wxc7fbca5e6ed8931f]]></ToUserName>" +
//					"<ItemCount>2</ItemCount>" +
//					"<PackageId>429496734063374065</PackageId>" +
//					"<Item>" +
//						"<FromUserName><![CDATA[xiongl]]></FromUserName>" +
//						"<CreateTime>1477203318</CreateTime>" +
//						"<MsgType><![CDATA[text]]></MsgType>" +
//						"<Content><![CDATA[11]]></Content>" +
//						"<MsgId>6195839788881033228</MsgId>" +
//						"<Receiver>" +
//							"<Type>userid</Type>" +
//							"<Id>xiongl</Id>" +
//						"</Receiver>" +
//					"</Item>" +
//					"<Item>" +
//						"<FromUserName><![CDATA[xiongl1]]></FromUserName>" +
//						"<CreateTime>1577203318</CreateTime>" +
//						"<MsgType><![CDATA[text]]></MsgType>" +
//						"<Content><![CDATA[12]]></Content>" +
//						"<MsgId>6195839788881033229</MsgId>" +
//						"<Receiver>" +
//						"<Type>userid</Type>" +
//						"<Id>xiongl</Id>" +
//						"</Receiver>" +
//					"</Item>" +
//					"</xml>";
			KfInMsg msg = KfInMsgParser.parse(s);
			renderText(msg.getPackageId());

			for (KfInBaseMsg m : msg.getItemList()) {
				dispatchKfMsg(m);
			}
			return;
		} catch (Exception e) {
			log.error("客服系统解析错误" + e);
			return;
		}
		}
	...
	private void dispatchKfMsg(KfInBaseMsg msg) {
		if (msg instanceof KfInTextMsg)
			processKfInTextMsg((KfInTextMsg) msg);
		else if (msg instanceof KfInImageMsg)
			processKfInImageMsg((KfInImageMsg) msg);
		else if (msg instanceof KfInVoiceMsg)
			processKfInVoiceMsg((KfInVoiceMsg) msg);
		else if (msg instanceof KfInFileMsg)
			processKfInFileMsg((KfInFileMsg) msg);
		else
			log.error("未能识别的消息类型。 消息 xml 内容为：\n" + getInMsgXml());
	}

	private void dispatchMsg(InMsg msg) {
	...
	// 客服消息
	// 处理接收到的文本消息
	protected abstract void processKfInTextMsg(KfInTextMsg inTextMsg);

	// 处理接收到的图片消息
	protected abstract void processKfInImageMsg(KfInImageMsg inImageMsg);

	// 处理接收到的语音消息
	protected abstract void processKfInVoiceMsg(KfInVoiceMsg inVoiceMsg);

	// 处理接收到的视频消息
	protected abstract void processKfInFileMsg(KfInFileMsg inFileMsg);
}

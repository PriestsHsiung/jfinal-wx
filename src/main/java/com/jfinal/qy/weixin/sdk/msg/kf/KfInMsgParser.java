...
package com.jfinal.qy.weixin.sdk.msg.kf;
...
public class KfInMsgParser {

	private KfInMsgParser() {}
	
	/**
	 * 从 xml 中解析出各类消息与事件
	 */
	public static KfInMsg parse(String xml) {
		try {
			return doParse(xml);
	 ...
	 * 7：event 事件
	 */
	private static KfInMsg doParse(String xml) throws DocumentException {
		Document doc = DocumentHelper.parseText(xml);
		Element root = doc.getRootElement();
		String toUserName = root.elementText("ToUserName");
		String packageId = root.elementText("PackageId");
		String agentType = root.elementText("AgentType");
		String itemCount = root.elementText("ItemCount");

		KfInMsg msg = new KfInMsg();
		msg.setToUserName(toUserName);
		msg.setPackageId(packageId);
		msg.setAgentType(agentType);
		msg.setItemCount(Integer.valueOf(itemCount));
		...
		for (Iterator iter = root.elementIterator("Item"); iter.hasNext();) {
			...
			String creatTime = e.elementText("CreateTime");

			String msgId = e.elementText("MsgId");

			Element recviver = e.element("Receiver");
			String type = recviver.elementText("Type");
			String id = recviver.elementText("Id");
			...
			t.setMsgId(msgId);
			t.getReceiver().setType(type);
			t.getReceiver().setId(id);
			msg.getItemList().add(t);
			msg.setItemCount(msg.getItemList().size());
		}
